import React, { useState } from 'react';

import Modal from 'components/modal';
import Button from 'components/button';

import bugIcon from 'assets/bug.svg';
import style from './remove-confirm-modal.module.scss';
import BugIcon from 'components/icon-component/bug-icon';

const ReportBug = ({
  openDelModal,
  setOpenDelModal,
  closeHandler = () => {},
  setModalDismissed,
  clickHandler,
  backClass,
  locked = false,
  name,
}) => {
  return (
    <Modal
      open={openDelModal}
      handleClose={() => {
        setOpenDelModal(false);
        setModalDismissed(true);
      }}
      className={style.mainDiv}
      backClass={backClass}
    >
      <div className={style.iconRefresh}>
        <BugIcon />
      </div>

      <p className={style.modalTitle}>Do you want to report bug for this failed test case ?</p>
      {locked && <p className={style.modalError}>Update Your Package to Report Bug</p>}
      <div className={style.mainBtnDiv}>
        <Button
          text={locked ? 'No, Fail Test Case' : 'Discard'}
          type={'button'}
          btnClass={style.btnClassUncheckModal}
          handleClick={async () => {
            locked && clickHandler();
            await closeHandler();
            setOpenDelModal(false);
            setModalDismissed(true);
          }}
          data-cy = "testcase-reportabug-discard-btn"

        />
        <Button text={'Yes, Report a Bug'} disabled={locked} handleClick={clickHandler} data-cy = "testcase-reportabug-btn" />
      </div>
    </Modal>
  );
};

export default ReportBug;
